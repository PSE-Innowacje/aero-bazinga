...
- [ ] **AUTH-03**: Access to menu sections and data is restricted by role per permissions matrix
...
- [ ] **OPS-05**: Planner can edit operations in statuses 1, 2, 3, 4, 5; planner cannot edit: auto-calculated fields, planned dates, status, post-completion notes
...
- [ ] **OPS-09**: Status auto-transitions: 3→4 when operation is selected for a flight order; 4→5, 4→6, or 4→3 when pilot reports flight completion
- [ ] **OPS-10**: Every field change is recorded in history log: old value, new value, timestamp, user
- [ ] **OPS-11**: Multiple comments can be appended to an operation (comment text up to 500 chars each, stored as ordered list)
...
- [ ] **FLT-01**: Pilot can create a flight order with: auto-assigned order number, planned start datetime, planned end datetime, pilot (auto-filled with current user), helicopter (active only), crew members (multi-select, optional), start airfield, end airfield, selected planned operations (multi-select, status 3 only, sorted by planned earliest date)
...
- [ ] **FLT-11**: When operations are added to a flight order, their status automatically changes from 3 to 4
